...
- 动态分区分配
    ...
    - 分区分配算法
        - 为了将一个作业装入内存，应按照- -定的分配算法从空闲分区表(或空闲分区链)中选出一个满足作业需求的分区分配给作业。如果这个空闲分区的容量比作业申请的空间容量要大，那么将该分区的一部分分配给作业，剩下的一部分仍然留在空闲分区表( 或空闲分区链)中，同时需要对空闲分区表(或空闲分区链)中的有关信息进行修改。目前常用的分配算法有以下4种。
